...
2. __Data Acquisition__: Obtain a suitable dataset. It must contain at least 50 observations and 2 quantitative variables.
    ...
    * You are also welcome to use a pre-established dataset
        * Some ideas are available on the Dataset Sources page
...
2. __Probability Calculation__ (Python or Excel):
    * Assuming the chosen variable is approximately normally distributed, or using the data’s calculated mean (μ) and standard deviation (σ):
        ...
        * Use the `scipy.stats` package (specifically `norm.cdf` and/or `norm.ppf`) to find the __probability__ of an observation falling within a specific range ($P(a<X<b)$) and/or the value corresponding to a specific percentile.
...
## Submission
Include 3 things in your submission:
* A copy of your data (an excel spreadsheet will work)
* A copy of your code
    * In your Google Colab project, click "File" -> "Download" -> ".ipynb"
* Your report (as a pdf or a doc/docx)